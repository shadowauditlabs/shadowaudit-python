...
"$python_cmd" -m shadowaudit.cli check "shadowaudit-demo/src"

printf "\n[STEP] Generate HTML assessment report\n"
set +e
"$python_cmd" -m shadowaudit.cli assess "shadowaudit-demo/src" --taxonomy financial \
  --output "shadowaudit-demo/shadowaudit-assessment-report.html"
assess_status=$?
set -e
...
audit_path = "shadowaudit-demo/audit.db"
...
"$python_cmd" -m shadowaudit.cli simulate --trace-file "tests/corpus/benign_traces.jsonl" --taxonomy general

printf "\n[STEP] Verify audit log hash chain\n"
"$python_cmd" -m shadowaudit.cli verify --audit-log "shadowaudit-demo/audit.db"

printf "\n[STEP] Validate a sample policy file\n"
"$python_cmd" -m shadowaudit.cli check-policy "policies/production_shell_policy.yaml"

printf "\n[DEMO] Done. Review the HTML report in shadowaudit-demo/\n"
