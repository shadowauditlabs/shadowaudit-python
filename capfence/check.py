...
def _find_capfence_wrappers(tree: ast.AST) -> set[str]:
    """Find which tool classes are wrapped with CapFenceTool.
    
    Detects patterns like:
        safe_tool = CapFenceTool(tool=SomeTool(), ...)
        safe_tool = CapFenceTool(tool=SomeTool, ...)  # class reference
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Call):
            ...
            # Direct: CapFenceTool(...)
            if isinstance(func, ast.Name) and func.id == "CapFenceTool":
                func_name = "CapFenceTool"
            # Imported: capfence.framework.langchain.CapFenceTool(...)
            elif isinstance(func, ast.Attribute) and func.attr == "CapFenceTool":
                func_name = "CapFenceTool"
...
def scan_file(
    ...
    *,
    ...
    local_wrapped = _find_capfence_wrappers(tree)
...
def _collect_all_wrappers(
    ...
    """Collect all CapFenceTool-wrapped class names across multiple files.
    ...
    for pyfile in pyfiles:
        ...
        if tree is None:
            continue
        all_wrapped |= _find_capfence_wrappers(tree)
    return all_wrapped
...
def scan_directory(
    ...
    *,
    ...
    1. First pass: collect all CapFenceTool-wrapped class names
