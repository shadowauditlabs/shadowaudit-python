        ...
        for tool in self.tools:
            if tool.needs_action:
                steps.append({
                    ...
                    "code": (
                        f"from capfence.framework.langchain import CapFenceTool\n"
                        f"{tool.name.lower()}_safe = CapFenceTool(\n"
                        f"    tool={tool.name}(),\n"
...
def enrich_finding(
    ...
    if finding.is_wrapped:
        remediation = "Protected by CapFenceTool"
    else:
        remediation = (
            f"Wrap with CapFenceTool: "
            f'CapFenceTool(tool={finding.name}(), agent_id="YOUR_AGENT_ID", '
