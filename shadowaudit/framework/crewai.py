...
__all__ = ["ShadowAuditCrewAITool", "AgentActionBlocked"]


class ShadowAuditCrewAITool(_GuardedToolMixin):
    ...
    async def arun(self, tool_input: str | dict[str, Any], **kwargs: Any) -> Any:
        """Async variant. Runs the gate check off the event loop and awaits
        the wrapped tool's ``arun`` if available."""
        payload = self._build_payload(tool_input)
