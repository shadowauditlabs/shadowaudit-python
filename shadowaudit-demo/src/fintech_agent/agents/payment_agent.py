...
Wraps PaymentTool and RefundTool with ShadowAuditTool.
...
def build_payment_agent():
    ...
    oversight ShadowAudit's scanner catches in CI.
    """
    safe_payment = ShadowAuditTool(
        tool=PaymentTool(),
        agent_id="payment-agent-1",
        risk_category="payment_initiation",
    )

    safe_refund = ShadowAuditTool(
        tool=RefundTool(),
        agent_id="payment-agent-1",
        risk_category="payment_initiation",
    )
    ...
    return {
        ...
        "wire_transfer": wire_transfer,  # UNGATED — ShadowAudit will flag this
    }
